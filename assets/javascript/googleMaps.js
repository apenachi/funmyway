console.log('loading GoogleMaps.js');
	...
	// Create a variable to reference the database
		...
		function searchPOI(poi){
			var wikiUrl = 'https://crossorigin.me/' + 'https://en.wikipedia.org/w/api.php?action=opensearch&search=' + poi + '&format=json&callback=wikiCallbackFunction';
			$.ajax({
				url : wikiUrl,
				crossDomain: true,
				type : 'GET',
		...
		function findPointOfInterest(lat, lng) {
			var loc = lat + ',' + lng;
			var ExpUrl = 'https://crossorigin.me/' + 'https://terminal2.expedia.com/x/geo/features?within=2km&lng=' + lng+ '&lat=' + lat + '&type=point_of_interest&apikey=' + ExpKey;
			$.ajax({
					url : ExpUrl,
					crossDomain: true,
					type: 'GET',
